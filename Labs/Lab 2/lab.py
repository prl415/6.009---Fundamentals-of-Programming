...
def get_movie_path(data, actor_id):
    '''
    Returns the movie path that connects actor with given actor_id and kevin bacon if there exists such a path
    ...
    Arguments:
        ...
        actor_id: integer, id of the actor
    '''
    actor_path = get_bacon_path(data, actor_id)
    if actor_path != None:
        movie_path = {}
        print(actor_path)
        for i in range(1, len(actor_path)):
            for e in data:
                if set(e[:2]) == set(actor_path[i-1:i+1]):
                    for k in id_to_movies:
                        if id_to_movies[k] == e[2]:
                            print(actor_path[i-1:i+1], k)
                            movie_path[tuple(actor_path[i-1:i+1])] = k
                        
        return list(movie_path.values())
    return None
