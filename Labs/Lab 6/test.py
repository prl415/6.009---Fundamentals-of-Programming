...
class Test_2_TupleTrie(unittest.TestCase):
    ...
    def test_04_iter(self):
        ...
        expected = [('b', 1), ('ba', 2), ('bar', 3), ('bark', 4), ('bat', 5), ('bath', 6)]
...
class Test_4_AutoComplete(unittest.TestCase):
    ...
    def test_06_type_check_autocomplete(self):
        #String
        trie = lab.make_word_trie("b ba ba bar bar bar bark bark bat bat bat bat bath")
        result = lab.autocomplete(trie, 'ba', 3)
        self.assertIsInstance(result,list,"result not a list.")
        for w in result:
            self.assertIsInstance(w,str,"expecting list of strings.")
        result.sort()
        expect_one_of = [["ba","bar", "bat"],["bar","bark", "bat"]]
        self.assertIn(result,expect_one_of,msg="incorrect result from autocomplete.")
        ...
        t[(-1, -2, -3)] = 2
        t[(2, )] = 3
        result = lab.autocomplete(t, (7, 8), 3)
        self.assertIsInstance(result,list,"result not a list.")
        for w in result:
            self.assertIsInstance(w,tuple,"expecting list of tuples.")
        result.sort()
        expect_one_of = [[(7, 8, 9), (7, 8, 9, 'hello'), (7, 8, 9, 'hello', (1, 2))],[(7, 8, 9), (7, 8, 9, 'hello'), (7, 8, 9, (1, 2), 'hello')]]
        self.assertIn(result,expect_one_of,msg="incorrect result from autocomplete.")
